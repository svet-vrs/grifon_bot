...
async def final_question(call: types.CallbackQuery, state=FSMContext):
    ...
    if call.data == "finish_yes":
        ...
        await bot.send_message(chat_id=config.CHAT_ID, text="🔔 Поступила заявка на операцию с недвижимостью \n\n🔹 ID заявки: "+str(data['user_id'])+"\n🔸 ФИО: "+str(data['name'])+"\n🔹 Номер: "+(data['phone_num'])+"", parse_mode='html', reply_markup=kbru.details_markup)
